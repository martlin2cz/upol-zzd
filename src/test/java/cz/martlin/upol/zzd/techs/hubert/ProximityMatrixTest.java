package cz.martlin.upol.zzd.techs.hubert;

import static org.junit.Assert.*;

import java.util.HashSet;
import java.util.Iterator;
import java.util.Set;

import org.junit.Test;

import cz.martlin.upol.zzd.common.abstracts.DistanceMeasure;
import cz.martlin.upol.zzd.common.abstracts.MergeDistComputer;
import cz.martlin.upol.zzd.datasets.ints.BasicIntsDistance;
import cz.martlin.upol.zzd.datasets.ints.IntDataObject;
import cz.martlin.upol.zzd.datasets.ints.IntsAvgMerger;
import cz.martlin.upol.zzd.techs.clustering.Cluster;
import cz.martlin.upol.zzd.techs.proximity.ProximityMatrix;
import cz.martlin.upol.zzd.utils.Utils;

public class ProximityMatrixTest {

	private static final double EPSILON = 0.001;

	@Test
	public void testMergingOfFirst() {
		ProximityMatrix<IntDataObject> mx1 = createFirst();
		mx1.print(System.out);

		Cluster<IntDataObject> col1 = Utils.createSingleton(new IntDataObject(9));
		Cluster<IntDataObject> col2 = Utils.createSingleton(new IntDataObject(25));
		mx1.mergeClusters(col1, col2);
		mx1.print(System.out);

		Cluster<IntDataObject> col3 = Utils.createSingleton(new IntDataObject(4));
		Cluster<IntDataObject> col4 = Utils.createSingleton(new IntDataObject(10));
		mx1.mergeClusters(col3, col4);
		mx1.print(System.out);

		Cluster<IntDataObject> col5 = Utils.mergeClusters(col1, col2);
		Cluster<IntDataObject> col6 = Utils.mergeClusters(col3, col4);
		mx1.mergeClusters(col5, col6);
		mx1.print(System.out);
	}

	@Test
	public void testMergingOfSecond() {
		ProximityMatrix<IntDataObject> mx2 = createSecond();
		mx2.print(System.out);

		Cluster<IntDataObject> col1 = Utils.createSingleton(new IntDataObject(4));
		Cluster<IntDataObject> col2 = Utils.createSingleton(new IntDataObject(6));
		mx2.mergeClusters(col1, col2);
		mx2.print(System.out);

		Cluster<IntDataObject> col3 = Utils.createSingleton(new IntDataObject(8));
		Cluster<IntDataObject> col4 = Utils.createSingleton(new IntDataObject(9));
		mx2.mergeClusters(col3, col4);
		mx2.print(System.out);

		Cluster<IntDataObject> col5 = Utils.createSingleton(new IntDataObject(13));
		Cluster<IntDataObject> col6 = Utils.mergeClusters(col3, col4);
		mx2.mergeClusters(col5, col6);
		mx2.print(System.out);

		Cluster<IntDataObject> col7 = Utils.mergeClusters(col5, col6);
		Cluster<IntDataObject> col8 = Utils.mergeClusters(col1, col2);
		mx2.mergeClusters(col7, col8);
		mx2.print(System.out);
	}

	@Test
	public void testSomeStuffOfFirst() {
		ProximityMatrix<IntDataObject> mx1 = createFirst();

		Cluster<IntDataObject> col1 = Utils.createSingleton(new IntDataObject(9));
		Cluster<IntDataObject> col2 = Utils.createSingleton(new IntDataObject(25));
		assertEquals(0.0, mx1.getAt(col1, col1), EPSILON);
		assertEquals(0.0, mx1.getAt(col2, col2), EPSILON);
		assertEquals(16.0, mx1.getAt(col1, col2), EPSILON);
		assertEquals(16.0, mx1.getAt(col2, col1), EPSILON);

		Iterator<ClustersTuple<IntDataObject>> iter = mx1.iterator();
		assertEquals(0.0, mx1.getAt(iter.next()), EPSILON);
		assertEquals(5.0, mx1.getAt(iter.next()), EPSILON);
		assertEquals(21.0, mx1.getAt(iter.next()), EPSILON);
		assertEquals(6.0, mx1.getAt(iter.next()), EPSILON);

		assertEquals(0.0, mx1.getAt(iter.next()), EPSILON);
		assertEquals(16.0, mx1.getAt(iter.next()), EPSILON);
		assertEquals(1.0, mx1.getAt(iter.next()), EPSILON);

		assertEquals(0.0, mx1.getAt(iter.next()), EPSILON);
		assertEquals(15.0, mx1.getAt(iter.next()), EPSILON);

		assertTrue(iter.hasNext());
		assertEquals(0.0, mx1.getAt(iter.next()), EPSILON);

		assertFalse(iter.hasNext());
		assertNull(iter.next());

		// Cluster<IntDataObject> col3 = Utils.createSingleton(new
		// IntDataObject(4));
		// Cluster<IntDataObject> col4 = Utils.createSingleton(new
		// IntDataObject(10));
		// mx1.mergeClusters(col3, col4);
		// mx1.print(System.out);
		//
		// Cluster<IntDataObject> col5 = Utils.mergeClusters(col1, col2);
		// Cluster<IntDataObject> col6 = Utils.mergeClusters(col3, col4);
		// mx1.mergeClusters(col5, col6);
		// mx1.print(System.out);
	}

	private ProximityMatrix<IntDataObject> createFirst() {
		Set<IntDataObject> set = new HashSet<>();

		set.add(new IntDataObject(10));
		set.add(new IntDataObject(4));
		set.add(new IntDataObject(9));
		set.add(new IntDataObject(25));

		DistanceMeasure<IntDataObject> distances = new BasicIntsDistance();
		MergeDistComputer merger = new IntsAvgMerger();
		// new MinMergeDistComputer();
		return new ProximityMatrix<>(set, distances, merger);
	}

	private ProximityMatrix<IntDataObject> createSecond() {
		Set<IntDataObject> set = new HashSet<>();

		for (int i = 0; i < 15; i++) {
			set.add(new IntDataObject(i));
		}

		DistanceMeasure<IntDataObject> distances = new BasicIntsDistance();
		MergeDistComputer merger = new IntsAvgMerger();
		// new MinMergeDistComputer();
		return new ProximityMatrix<>(set, distances, merger);
	}

}
